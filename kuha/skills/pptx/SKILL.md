...
- Run every script here as `python3 {baseDir}/scripts/<name>.py ...` (Windows: `py -3.12 ...`, not `python`/`python3`). Confirmed working on Windows: `add_slide.py`, `clean.py`, `office/validate.py` — pure Python, no external binary.
- Where a snippet says `python3 -c "..."`, that already works as written on macOS/Linux; on Windows use `py -3.12 -c "..."` instead.
- **No `zip`/`unzip` CLI on plain Windows** (macOS/Linux ship both). For the pack step (`(cd unpacked && zip -Xr ../out.pptx .)`) on Windows, use: `py -3.12 -c "import shutil,os; os.chdir('unpacked'); shutil.make_archive('../out','zip','.'); os.replace('../out.zip','../out.pptx')"`. `unzip` itself is present if you're in Git Bash; otherwise extract with `py -3.12 -c "import zipfile; zipfile.ZipFile('deck.pptx').extractall('unpacked')"`.
- `scripts/thumbnail.py` và mọi script dùng `office/soffice.py` chạy được trên cả Windows và macOS: bản Kuha đã vá `soffice.py` để bỏ bước dò socket Linux và tự tìm soffice ở các vị trí thường gặp (Windows: `C:\Program Files\LibreOffice\program\soffice.exe`; macOS: `/Applications/LibreOffice.app/Contents/MacOS/soffice`). Cần cài LibreOffice (Windows: `winget install TheDocumentFoundation.LibreOffice`; macOS: `brew install --cask libreoffice`). Không có `pdftoppm` (Poppler) mặc định trên máy nào; muốn xem ảnh từng slide thì convert sang PDF rồi dùng PyMuPDF (`python3 -m pip install pymupdf`; Windows: `py -3.12 -m pip install pymupdf`).
- **LibreOffice on Windows** is already installed at `C:\Program Files\LibreOffice\program\soffice.exe` (also on PATH as `soffice` in Git Bash) — needed only for PDF export and visual QA renders, not for building or editing a deck. Otherwise: `winget install TheDocumentFoundation.LibreOffice` (Windows) or `brew install --cask libreoffice` (macOS).
- **`pptxgenjs` is NOT globally installed** here (checked with `npm ls -g` on Windows) — the "write a pptxgenjs script and `require('pptxgenjs')`" path needs `npm i -g pptxgenjs` (and `react-icons`, `react`, `react-dom`, `sharp` for the icon-rendering gotcha) first. Until then, prefer the **python-pptx** path — verified working on Windows — for slide creation instead of pptxgenjs.
- **`markitdown` is not installed** (`python3 -m pip install markitdown`; Windows: `py -3.12 -m pip install markitdown`) if you want the CLI text dump. Without it, read slide text back with python-pptx (`for shape in slide.shapes: shape.text_frame.text`).
- **Vietnamese text**: always set the font explicitly to `"Arial"`, `"Calibri"`, or `"Times New Roman"` on every run/paragraph — these ship with Windows and render Vietnamese diacritics (crossed đ, ơ/ư with horn, all tone marks) correctly; on macOS these same names resolve via Office/LibreOffice's font substitution (see the `pdf` skill's font notes for local TTF paths if you need to register one directly). Do not rely on a template's default theme font for Vietnamese runs without checking it.
