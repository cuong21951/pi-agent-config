...
- Run every script here as `py -3.12 {baseDir}/scripts/<name>.py ...` (not `python`/`python3`). Confirmed working on this machine: `add_slide.py`, `clean.py`, `office/validate.py` — pure Python, no external binary.
- Where a snippet says `python3 -c "..."`, use `py -3.12 -c "..."` instead.
- **No `zip`/`unzip` CLI on plain Windows.** For the pack step (`(cd unpacked && zip -Xr ../out.pptx .)`), use: `py -3.12 -c "import shutil,os; os.chdir('unpacked'); shutil.make_archive('../out','zip','.'); os.replace('../out.zip','../out.pptx')"`. `unzip` itself is present if you're in Git Bash; otherwise extract with `py -3.12 -c "import zipfile; zipfile.ZipFile('deck.pptx').extractall('unpacked')"`.
- `scripts/thumbnail.py` và mọi script dùng `office/soffice.py` chạy được trên Windows: bản Kuha đã vá `soffice.py` để bỏ bước dò socket Linux và tự tìm `C:\Program Files\LibreOffice\program\soffice.exe`. Cần cài LibreOffice (`winget install TheDocumentFoundation.LibreOffice`). Không có `pdftoppm` (Poppler) ở đây; muốn xem ảnh từng slide thì convert sang PDF rồi dùng PyMuPDF (`py -3.12 -m pip install pymupdf`).
- **LibreOffice is already installed** on this machine at `C:\Program Files\LibreOffice\program\soffice.exe` (also on PATH as `soffice` in Git Bash) — needed only for PDF export and visual QA renders, not for building or editing a deck. Otherwise: `winget install TheDocumentFoundation.LibreOffice`.
- **`pptxgenjs` is NOT globally installed** here (checked with `npm ls -g`) — the "write a pptxgenjs script and `require('pptxgenjs')`" path needs `npm i -g pptxgenjs` (and `react-icons`, `react`, `react-dom`, `sharp` for the icon-rendering gotcha) first. Until then, prefer the **python-pptx** path — verified working on this machine — for slide creation instead of pptxgenjs.
- **`markitdown` is not installed** (`py -3.12 -m pip install markitdown` if you want the CLI text dump). Without it, read slide text back with python-pptx (`for shape in slide.shapes: shape.text_frame.text`).
- **Vietnamese text**: always set the font explicitly to `"Arial"`, `"Calibri"`, or `"Times New Roman"` on every run/paragraph — these ship with Windows and render Vietnamese diacritics (crossed đ, ơ/ư with horn, all tone marks) correctly. Do not rely on a template's default theme font for Vietnamese runs without checking it.
