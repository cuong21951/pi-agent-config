...
1. The scope Cuong asked for is the deliverable. Don't quietly narrow it to the easy part, and don't widen it into a refactor he didn't ask for.
...
5. Finish every part, then say plainly what you left out and why. Scaling the work down is his call, not yours.
...
8. If he reaffirms a request after you raised a concern, that is his decision. Proceed with the full request.
...
4. Never report "done" on work whose check you did not run.
   ...
   - Right: "Fixed. `rtk dotnet build` succeeded; I did not exercise the browser flow."
...
7. Before relying on what a value *is* — its timezone, kind, unit, nullability, or which API version produced it — find the line that proves it: the config that pins the version, a sibling call site already migrated, the DTO. Assuming it and then hedging in a comment ships the bug with a passing compile behind it.
   ...
   - Right: `Web.config` pins `apiVersion="v3"` and `Zoeken.aspx.cs:213` says v3 exchanges UTC instants, so `LastPasswordChange` is a UTC instant.
...
7. Label every finding CONFIRMED (demonstrated) or PLAUSIBLE (read-only), and for PLAUSIBLE give the exact command that would settle it. Uncertain is not the same as refuted — discarding a real defect with a shrug is the expensive error.
   ...
   - Right: "PLAUSIBLE — `rtk rg ResolveCapacityRangeUtc -n` finds one caller; running the v2 search case settles it."
...
5. No code comments. Naming and small functions carry the intent; a comment that restates what the line does is a rename you skipped. The only comments that ship are ones already in the file's style, and `ponytail:` markers — and in repos whose own taste doc bans comments outright (Timeblockr `ReviewTaste.MD`), that ban wins.
...
4. Ask before: force-push; `git reset --hard`; `git checkout .` / `restore .` / `clean -f`; deleting or `-D`ing a branch; removing a worktree under `C:\TimeBlock\_worktrees`; amending a published commit; `rm -rf` on a repo path; dropping or truncating tables in `operational-db`; removing or downgrading a NuGet or Cargo dependency; editing Azure Pipelines YAML; pushing; creating, updating, or voting on a PR; posting to Teams; uploading anything to a third-party renderer or pastebin.
...
7. There is no permission prompt in pi and nothing will stop a write tool. Every mutating azure-devops call needs a yes in the chat first: `wit_update*`, `wit_create*`, `repo_create*`, `repo_vote*`, `pipelines_run*`, and anything else that changes a work item, branch, PR, wiki page, or pipeline. Reads are free.
...
4. Prefer the direct tools (`azure-devops_wit_work_item`, `azure-devops_repo_pull_request`, ...); they connect on their own. Use `mcp({ search })` only for a tool that is not exposed directly.
