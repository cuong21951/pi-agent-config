...
const LOOP_REASON =
	...
	"If you are waiting on something, poll with a different command or ask Cuong. " +
...
const LOOP_ESCALATION =
	...
	"and hand control back to Cuong.";
...
function destructiveReason(command: string): string {
	return (
		...
		"If you believe the working tree is already clean, prove it with `git status --short` and say so to Cuong before retrying. " +
...
function imageReason(capable: readonly string[]): string {
	return (
		...
		(capable.length > 0
			? `Switch to an image-capable model with Ctrl+P (${capable.join(", ")}) and read the file there, or ask Cuong to describe it. `
			: "No image-capable model is configured in this pi install, so there is nothing to switch to. Tell Cuong plainly that you cannot see the image and ask him to describe it or to run this task in Claude Code. ") +
...
if (process.env.DEEPSEEK_GUARDS_SELFTEST) {
	...
	check(
		isUnboundedScan("find / -name \"NuGet.config\" 2>/dev/null | head -3; find /c/Users/cuong -maxdepth 2 -name x"),
