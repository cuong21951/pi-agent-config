...
// ponytail: Claude Code's footer is one line of " · " separated facts starting with the model;
...
export function composeFooter(f: FooterFacts, paint: Paint, maxWidth?: number): string {
	...
	let parts = [paint("accent", f.model)];
...
export function composeModeRow(mode: string, width: number, voice?: string): string {
	if (!voice) return mode;
	const gap = width - plainWidth(mode) - plainWidth(voice);
	return gap < 1 ? mode : `${mode}${" ".repeat(gap)}${voice}`;
}

// ponytail: Claude's footer has no such badge, so the ponytail status is dropped rather than rendered.
export function badge(status: string, paint: Paint): string {
	return isPonytail(status) ? "" : status.trim();
}
		...
		ctx.ui.setFooter((_tui, theme, footerData) => {
			...
			return {
				render(fullWidth: number) {
					...
					const rows = mode ? [line, composeModeRow(mode, width, statuses.get(VOICE_STATUS))] : [line];
...
if (process.env.CLAUDE_FOOTER_SELFTEST) {
	...
	const base: FooterFacts = { statuses: [], model: "GLM 5.3 Flash", thinking: "high", contextPercent: 22.4, cost: 0.284 };
	check(composeFooter(base, plain) === "GLM 5.3 Flash · think high · ctx 22% · $0.28", "full line");
	...
	check(
		composeFooter({ ...base, statuses: ["deepseek $24.57", "MCP 1/11"], branch: "master" }, plain) ===
			"GLM 5.3 Flash · think high · ctx 22% · $0.28 · master · deepseek $24.57 · MCP 1/11",
		"statuses trail the facts",
	);
	...
	const crowded = { ...base, statuses: ["deepseek $24.57", "openrouter $20.38", "MCP 1/11"], branch: "master", model: "DeepSeek V4 Flash Vision Exp" };
	const at120 = composeFooter(crowded, plain, 110);
	check(at120.includes("ctx 22%") && at120.includes("$0.28") && at120.includes("master"), "110 columns keep ctx, cost and branch");
	check(!at120.includes("MCP 1/11") && at120.length <= 110, "110 columns drop the trailing statuses first");
	const narrow = composeFooter(base, plain, 30);
	check(narrow === "GLM 5.3 Flash · think high", "overflow drops whole parts");
	...
	check(badge("\x1b[32m● ponytail: ⚡ FULL\x1b[0m", tagged) === "", "the ponytail status is dropped — Claude's footer has no badge");
