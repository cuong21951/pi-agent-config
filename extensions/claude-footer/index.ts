...
export function composeFooter(f: FooterFacts, paint: Paint): string {
	const parts = [...f.statuses, paint("accent", f.model)];
		...
		ctx.ui.setFooter((_tui, theme, footerData) => {
			...
			return {
				render(width: number) {
					...
					return [line.replace(ANSI, "").length > width ? `${line.slice(0, Math.max(0, width - 1))}…` : line];
				},
				invalidate() {},
			};
		});
