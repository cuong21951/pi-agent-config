...
```bash
python "C:/Users/cuong/.pi/agent/skills/search-youtube/scripts/yt_research.py" search "{TOPIC}" --count {YT_COUNT} --after $(date -v-{DAYS}d +%Y%m%d 2>/dev/null || date -d "{DAYS} days ago" +%Y%m%d)
```

For `deep` depth, or when ≥1 video looks highly relevant (based on title + view count): fetch transcript of the top 1-2 results:

```bash
python "C:/Users/cuong/.pi/agent/skills/search-youtube/scripts/yt_research.py" transcript "{VIDEO_URL}"
```
